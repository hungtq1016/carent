<template>
    ...
    class="hover:bg-gray-100 rounded-md px-2" :class="{'bg-gray-100':carStore.brand==brand.name}">
        ...
        <img v-else class="object-contain w-20 min-h-[80px] mix-blend-multiply" :src="IMG_URL + brand.image?.path"
            :alt="brand.name">
        <div class="flex gap-x-1 items-center">
            <div class="text-sm capitalize font-semibold">{{ brand.name }}</div>
...
const selectBrand = () =>{
    carStore.brand = props.brand.name
    emit('closeModal')
}
