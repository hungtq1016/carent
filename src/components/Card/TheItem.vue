<template>
    <div
        ...
        <div class="relative">
            <div v-if="isLoading"
                class="h-60 flex items-center justify-center bg-gray-300 dark:bg-zinc-800 rounded-md mx-2 animate-pulse">
            ...
            <RouterLink :to="`/car/${car.info.slug}/id/${car.id}`" v-else>
                <img :src="IMG_URL + car.image.path" :alt="car.info.name" loading="lazy"
                    class="object-cover rounded-md w-full h-[300px]">
            </RouterLink>
            <div class="absolute top-1 left-1">
                <button @click="findCarsBySeat(car.seats)"
        ...
        <div class="flex flex-col gap-y-2 justify-between flex-auto">
            <div class="flex flex-col gap-y-2">
                <div class="flex text-sm tags">
                    <div class="bg-amber-100 px-2 py-1 rounded-l dark:bg-amber-600"
                        :class="{ 'rounded-r': !car.isDelivery }">
                ...
                <div class="flex flex-col gap-y-1">
                    <div class="flex gap-x-2 items-center">
                        <div class="text-lg font-semibold capitalize dark:text-gray-200">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                            class="w-6 h-6 fill-green-700 " v-else>
                    ...
                    <div class="flex items-center gap-x-2">
                        ...
                        <div class="text-gray-600 text-sm dark:text-gray-100 cursor-pointer" @click="findCarsByLocation(car.location)">
                            <span v-if="car.location.district" class="text-gray-900 font-semibold dark:text-gray-200">{{
                                car?.location.district.name }},
                            </span>
                            <span v-if="car.location.province" class="text-gray-900 font-semibold dark:text-gray-200">{{
                                car?.location.province.name }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center">
                    <div class="flex flex-col gap-y-1">
                        <div class="flex items-center gap-x-1">
                            ...
                            <span class="text-xs text-gray-600 dark:text-gray-100">{{ Number(car.review).toFixed(1)
                            }}</span>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
...
const findCarsBySeat = (seat: number) => {
    router.push({
        name: 'FindCars',
    })
    carsStore.query.seat = seat
}

const findCarsByFuel = (fuel: number) => {
    router.push({
        name: 'FindCars',
    })
    carsStore.query.fuel_type = fuel
}

const findCarsByLocation = (location: any) => {
    router.push({
        name: 'FindCars',
    })
    carsStore.query.province = location.province.slug
    carsStore.query.district = location.district.slug ?? 'all'
    console.log(carsStore.query.district);
