    ...
    <RouterLink :to="`/car/${car.info.slug}/id/${car.id}`"
        ...
        <div class="relative">
            <div>
                ...
                <img v-else :src="IMG_URL+car.image.path" :alt="car.info.name"
        ...
        <div class="flex flex-col gap-y-2 justify-between flex-auto">
            <div class="flex flex-col gap-y-2">
                <div class="flex text-sm tags">
                    <div class="bg-amber-100 px-2 py-1 rounded-l" :class="{ 'rounded-r': !car.isDelivery }">
                        {{ car.transmission_type == 1 ? 'Số tự động' : 'Số sàn' }}
                    </div>
                    <div v-if="car.isDelivery" class="bg-green-100 px-2 py-1 rounded-r">
                        Giao tận nơi
                    </div>
                </div>
                <div class="flex flex-col gap-y-1">
                    ...
                    <div class="flex items-center gap-x-2">
                        ...
                        <div class="text-gray-600 text-sm">{{ car?.location }}</div>
                    </div>
                </div>
            </div>
            <div class="pt-2 border-t">
                <div class="flex justify-between items-center">
                    <div class="flex flex-col gap-y-1">
                        ...
                        <div class="flex items-center gap-x-1">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
                                    stroke="currentColor" class="w-4 h-4 stroke-rose-600">
                                    <path strokeLinecap="round" strokeLinejoin="round"
                                        d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                                </svg>
                            <span class="text-sm text-gray-600">{{ car?.like_count }}</span>
                        </div>
                    </div>
                    <div class="flex flex-col gap-y-1">
                        <div class="flex items-center gap-x-1 flex-row-reverse">
                            ...
                            <span class="text-xs text-gray-600">{{ car?.total_trip }}</span>
                        </div>
                        <div class="">
