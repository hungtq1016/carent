<template>
    <RouterLink :to="`/car/${car.info.slug}/id/${car.id}`"
        class="flex flex-col  p-4 rounded-md bg-white border gap-y-2 hover:shadow duration-300 dark:bg-slate-900 dark:border-gray-700">
        <div class="relative">
            <div v-if="isLoading" class="h-60 flex items-center justify-center bg-gray-300 rounded-md mx-2 animate-pulse">
                <svg class="w-10 h-10 text-gray-200 dark:text-slate-100" aria-hidden="true"
            ...
            <div class="flex flex-col gap-y-2">
                ...
                <div class="flex flex-col gap-y-1">
                    <div class="flex gap-x-2 items-center">
                        <div class="text-lg font-semibold capitalize dark:text-slate-200">{{ car.info.name }}</div>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
                    ...
                    <div class="flex items-center gap-x-2">
                        ...
                        <div class="text-gray-600 text-sm dark:text-slate-100">{{ car?.location }}</div>
                    </div>
                </div>
            </div>
                ...
                <div class="flex justify-between items-center">
                    <div class="flex flex-col gap-y-1">
                        <div class="flex items-center gap-x-1">
                            ...
                            <span class="text-xs text-gray-600 dark:text-slate-100">{{ Number(car.review).toFixed(1) }}</span>
                        </div>
                        <div class="flex items-center gap-x-1">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
                            ...
                            <span class="text-sm text-gray-600 dark:text-slate-100">{{ car?.like_count }}</span>
                        </div>
                    </div>
                    <div class="flex flex-col gap-y-1">
                        <div class="flex items-center gap-x-1 flex-row-reverse">
                            ...
                            <span class="text-xs text-gray-600 dark:text-slate-100">{{ car?.total_trip }}</span>
                        </div>
                        <div class="dark:text-slate-100 text-white">
