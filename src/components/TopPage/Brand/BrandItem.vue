<template>
    <RouterLink :to="`/find?brand=${brand.slug}`" class="h-fit px-2">
        ...
        :src="URL+brand.image.local_src" :alt="brand.name">
    </RouterLink>
</template>
...
const { isLoading ,error,isReady } = useImage({ src: URL+props.brand.image.local_src })
