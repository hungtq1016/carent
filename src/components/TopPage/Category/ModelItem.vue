<template>
    <RouterLink :to="`/find?brand=${model.slug}`" class="h-fit px-2">
        ...
        :src="`http://localhost:8000${model.image.local_src}`" :alt="model.name">
    </RouterLink>
</template>
...
const { isLoading ,error,isReady } = useImage({ src: `http://localhost:8000${props.model.image.local_src}` })
